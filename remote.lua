...
events.detect = function ()
	return 
		libs.fs.exists("C:\\Users\\admin\\AppData\\Local\\Vivaldi\\Application") or
		libs.fs.exists("C:\\Program Files\\Vivaldi");
end

--@help Focus Opera application
actions.switch = function()
	if OS_WINDOWS then
		local hwnd = win.window("vivaldi.exe");
		if (hwnd == 0) then actions.launch(); end
		win.switchtowait("vivaldi.exe");
	end
end
...
actions.launch = function()
	if OS_WINDOWS then
    	os.start("C:\\Users\\admin\\AppData\\Local\\Vivaldi\\Application\\vivaldi.exe");
	end
end
